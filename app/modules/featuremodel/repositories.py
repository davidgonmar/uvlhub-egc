...
class FeatureModelRepository(BaseRepository):
    ...
    def count_feature_models(self) -> int:
        count = self.model.query.with_entities(func.count(self.model.id)).scalar()
...
class FMRatingRepository(BaseRepository):
    ...
    def get_average_by_feature_model(self, feature_model_id: int) -> float:
        avg = self.model.query.filter_by(feature_model_id=feature_model_id).with_entities(func.avg(self.model.rating))
        return avg.scalar()
